...
async function fetchPlaceholderData() {
  ...
  const { aboutGallery, timeDelay, websiteName } = await import(
    "@/app/lib/placeholder-data"
  );
  
  return { aboutGallery, timeDelay, websiteName };
}


export const AboutSection: FC = async (): Promise<JSX.Element> => {
  // # FETCH DATA SIMULATED
  const { aboutGallery, timeDelay, websiteName } = await fetchPlaceholderData();
